...
static int hi259_vendor_id_read(int addr)
{
    ...
    no_printk("hynix_hi259_I read vendor id , form addr 0x%x is: 0x%x\n",addr,flag);
	return flag;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if(flag != hi259_vendor_id){
        no_printk("hynix_hi259_I  match vendor id fail, reead vendor id is: 0x%x,expect vendor id is 0x%x \n", flag,hi259_vendor_id);
